...
class PoseEstimator:
    """
    MediaPipe Pose estimator that extracts 33 landmarks per detected person.

    Mapping guarantee:
    - Each person ROI is cropped from the exact detection bounding box.
    - Landmark coordinates are transformed from ROI-normalized space back
      to full-frame pixel coordinates using the same box offsets.
    - Output order matches detection order, so box[i] <-> pose[i] is stable.
    """
    ...
    def estimate(self, frame: np.ndarray, boxes: list[BoundingBox]) -> list[PoseResult]:
        """Extract 33 pose keypoints for each detected person bounding box."""
        h, w = frame.shape[:2]
        ...
        for box in boxes:
            ...
            # Invalid or tiny ROI cannot produce reliable keypoints.
            if x2 <= x1 or y2 <= y1:
            ...
            mp_result = self._pose.process(roi_rgb)
            ...
            # MediaPipe Pose emits 33 landmarks; preserve that exact count.
            for lm in mp_result.pose_landmarks.landmark:
                px = int(x1 + (lm.x * roi_w))
                py = int(y1 + (lm.y * roi_h))

                # Clamp transformed points to frame bounds.
                px = max(0, min(px, w - 1))
                py = max(0, min(py, h - 1))
